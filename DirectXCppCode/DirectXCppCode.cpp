...
CPP_API void PrepareScene(int handle, int w, int h, int countFe, double* points)
{
	auto finded = device->find((HWND)handle);
	vector<vector<array<double, 3>>> arrayLayers;

	for (auto i = 0; i < countFe; i++)
	{
		vector<array<double, 3>> arrayPoints;
		for (auto j = 0; j < 8; j++)
		{
			array<double, 3> arrayPoint;
			for (auto k = 0; k < 3; k++)
			{
				arrayPoint[k] = points[j * 3 + 24 * i + k];
			}
			arrayPoints.push_back(arrayPoint);
		}
		arrayLayers.push_back(arrayPoints);
	}
	...
	if (finded != device->end())
	{
		...
		for (int j = 0; j < countFe; j++)   // countFe
		{
			...
			auto unit = finded->second->CreateLineColorUnit(lines, arrayLayers[j], normals);
...
CPP_API void RenderScene(int handle, double stepChanger, int changerX, int changerY, int changerZ, double coefDepth)
{
	...
	{
		finded->second->RenderStart();
		DirectX::XMMATRIX myViewMatrix = DirectX::XMMatrixTranslation(0.0f, 0.0f, 0.0f);
